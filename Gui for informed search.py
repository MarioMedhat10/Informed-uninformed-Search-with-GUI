...
class Game:
    ...
    def __init__(self, master):
        self.master = master
        self.master.title("Informed Search Game")
    ...
    def run_game(self):
        ...
        prolog.consult("assign 2 informed Search.pl") # load the Prolog code file

        # call the makeGame predicate and get the result
        result = list(prolog.query(f"makeGame({rows},{cols},{b1x},{b1y},{b2x},{b2y},MM,MaxDominos)"))
        ...
        for board in result:
            ...
            self.b2x_label = tk.Label(new_window, text="Max Dominos : ",justify='center')
            self.b2x_label.grid(row=rows+1, column=0)
            maximum = tk.Text(new_window,width=10,height=1)
            maximum.grid(row=rows+1,column=1)
            maximum.insert(tk.END,str(board["MaxDominos"]))
            maximum.config(fg="black",font=("Arial",10),state='disabled')
